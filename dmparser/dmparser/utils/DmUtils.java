...
public class DmUtils {
	...
	public static String getAssociationName(DataModel dm, String className, String endName) {
		...
		for (Association assoc : assocs) {
			if (assoc.getLeftEntityName().equals(className) && assoc.getRightEnd().equals(endName)) {
				return assoc.getName();
			}

			if (assoc.getRightEntityName().equals(className) && assoc.getLeftEnd().equals(endName)) {
				return assoc.getName();
			}
		}
	...
	public static boolean isAssocO2O(DataModel dm, String className, String endName) {
		return getAssocRelationship(dm, className, endName) == AssociationType.ONE_TO_ONE;
	}
...
//            e.printStackTrace();
	...
	public static Attribute getAttribute(DataModel dm, String className, String attName) {
		Entity entity = dm.getEntities().get(className);
	...
	public static boolean isSuperClassOf(DataModel dm, String typeToCheck, String referredType) {
		// TODO Will be consider later when we have generalization
		return false;
	}

	public static Entity getEntity(DataModel dataModel, String entityToRead) {
		return dataModel.getEntities().get(entityToRead);
	}

	public static Attribute getAttribute(Entity entity, String attributeToRead) {
		Set<Attribute> atts = entity.getAttributes();

		for (Attribute att : atts) {
			if (att.getName().equals(attributeToRead)) {
				return att;
			}
		}
	...
	public static Association getAssociation(DataModel dm, String sAssociation) {
	...
	public static Entity getUserEntity(DataModel dataModelFile) {
		return dataModelFile.getEntities().values().stream().filter(Entity::isUserClass).findFirst().orElseGet(null);
	}
