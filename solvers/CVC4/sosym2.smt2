...
; The predicate for the first temporary table
...
(assert (TEMP1 klecturers kstudents))

; the caller teaches all the students
(assert (forall ((s Classifier)) 
  (and (=> (Student s) 
           (exists ((temp Classifier)) 
             (and (Enrollment kcaller temp) 
                  (= temp s) 
